...
**Last updated:** 2026-08-03 · **`main` HEAD:** `d37df573` · **Active branch:** `fix/508-systemd-enable-units` · **Last gate:** DGX **2952 / 0 / 53**, `TEST_EXIT=0`, clippy `-D warnings` `CLIPPY_EXIT=0`, 4 `[SKIP]` (all the env-gated GLiNER tier)
...
`main` is at **`4e1d30ca`**; besides PR [#511](https://github.com/hherb/kastellan/pull/511) (this session, below) the only open kastellan PR is the unrelated dependabot [#453](https://github.com/hherb/kastellan/pull/453). Everything below the last three merges is settled: the VM-entry arc is COMPLETE, v0.2.0 is RELEASED, the audit-remediation family is FULLY closed, the mail-worker test/e2e arc is done, and the extra-CA capability, its operator config and its live confirmation all shipped.

**Kastellan survives a reboot on Linux** (`4e1d30ca`, PR [#509](https://github.com/hherb/kastellan/pull/509), closes [#508](https://github.com/hherb/kastellan/issues/508)). `supervisor/src/systemd_user.rs` wrote correct `[Install] WantedBy=` stanzas and then never ran `systemctl --user enable`, so nothing was ever linked into `default.target` and a rebooted host came up running nothing — while macOS came back, because `RunAtLoad=true` is unconditional in every plist. That parity break is what left a real Matrix message unanswered on 2026-08-02. `install` now enables `<name>.service`; `install_target` enables `<name>.target` **only** — it is the boot entry point, and its own `Wants=` line pulls the members in, so a second link for each member would be redundant (pinned by a test so a later change is deliberate); `uninstall_target` disables symmetrically, or a real uninstall would strand a `default.target.wants/` symlink. Both calls sit behind the existing `is_default_units_dir()` guard, and `enable` failures propagate rather than being swallowed — a silent failure here reproduces the exact bug, and the operator would not find out until the next reboot. The `Supervisor` trait now carries **"install implies auto-start"** as an explicit cross-platform contract. **Gate: DGX 2952 / 0 / 53.**

**Active branch `fix/509-review-followups` — PR [#511](https://github.com/hherb/kastellan/pull/511), the retrospective review of #509** (which was merged before it was reviewed). No bug in #509's fix — the systemd semantics, the enable-the-target-only choice, and the disable-before-remove ordering all check out. Six findings, five fixed here:

1. **`write_atomic`'s staging path was the real cause of the collision #509 patched one layer up.** `path.with_extension("service.tmp")` is a pure function of the *destination*, so any two concurrent writers of one unit race on a single tmp file — and because `with_extension` *replaces* the final component, a `.target` was staged through `<name>.service.tmp`, the same path a like-named `.service` would use. #509 made the smoke-test *names* unique, which stops the tests colliding but leaves the helper unsafe for its real caller (two concurrent `kastellan-cli install` runs). Now `tmp_path_for()` appends `.tmp.<pid>.<n>` to the whole file name, **and every error path removes the staging file** — mandatory once the name is unique, or a repeatedly-failing install litters the dir instead of overwriting one leftover. **Fixed in `launchd_agents.rs` too**: identical hazard, and parity is a hard constraint. (Superseded below: the two copies are now one shared module.)
2. **A stale comment in `builder.rs` contradicted the fix** — "We don't enable by default — that's a separate policy decision", 20 lines from the new module doc saying the opposite. The `[Install]` section is now load-bearing for reboot survival; the comment says so.
3. **The `--no-start` path doesn't deliver the contract, and only a maintainer could tell.** It returns before the linger call, so on a headless host the units are armed but no manager is up at boot to act on it. #509 explained this in a code comment; the operator sees a message, so the message now says it.
4. **The trait contract overstated what a backend can promise.** "Armed to start after a reboot" is false on *both* platforms for a host nobody logs into — launchd loads `~/Library/LaunchAgents/` at login, not at boot. Reworded to "when the user's service manager next comes up", with the host-level half (linger / a GUI session) named as explicitly out of scope.
5. **Crashed-test leftovers got worse and the cleanup guidance didn't keep up.** Guards cover a panic, not a SIGKILL — and now that `install` enables, a hard-killed run strands a symlink in the operator's real `default.target.wants/` that the user manager *fails to start on every login*, where a stray unit file was merely inert. Both smoke files carry the two-prefix sweep command.

The sixth is [#510](https://github.com/hherb/kastellan/issues/510), filed not fixed: **CI never exercises #508's regression guard.** `linux-check.yml` is compile-only plus hermetic `tests-common`; it never runs `cargo test -p kastellan-supervisor`, and both new tests would `[SKIP]` on `ubuntu-latest` anyway (no user session ⇒ no `systemctl --user`). So the only thing between `enable()` being deleted and #508 silently returning is an operator running the DGX suite — which is the failure mode #508 *was*. See [What CI does not cover](#what-ci-does-not-cover).

Also declined, deliberately: `is_enabled_state` is duplicated across the two smoke binaries, and `uninstall_target`'s `disable` is not behind `is_default_units_dir()` while `install_target`'s `enable` is. Both match pre-existing style in those files; churning them would enlarge the diff without changing behaviour.

**#511's own review round (2026-08-03) — the fix was right, and narrower than it read in three places.** Nothing above is wrong; each of the three shipped a correction rather than a retraction.

- **The duplication.** Finding 1 landed the same `tmp_path_for` + `write_atomic` + `write_and_sync` in *both* backends, byte-for-byte, in `cfg`-exclusive modules — and with no macOS runner in CI, the launchd copy is not even *compiled* on a PR. "A fix applied to one backend and not the other" is #508's failure class, so reproducing its shape here was the wrong trade. Now one `cfg`-free `supervisor/src/atomic_write.rs`, gated `any(linux, macos)` so it is never dead code on the `NotYetImplemented` fallback. The payoff is not brevity: its **seven tests run on both hosts**, where before the `.target`-collapses-onto-`.service` hazard was pinned only on Linux and the is-the-staging-file-loadable hazard only on macOS. Each backend keeps a wiring test proving `install` publishes through the helper.
- **The staging file is now created with `create_new(true)`,** so `tmp_path_for`'s uniqueness is enforced rather than assumed: a collision is a loud error instead of a silent clobber, and a symlink pre-planted at the staging path is refused (`O_EXCL`) rather than followed. The cleanup was also tightened — a failed *create* removes nothing, because at that point the path is not ours to delete.
- **The claim "two concurrent `kastellan-cli install` runs still race" stayed true after the fix**, one layer up: `copy_exec` and `symlink_replace` in `core/src/install/run.rs` staged through `dest.with_extension("tmp-install")` — the same destination-derived pattern, in the same install flow, for the binaries and the operator CLI symlink. `symlink_replace` even `remove_file`d that path first, which under concurrency deletes the *other* writer's staging link. Both now use a per-writer `staging_path()` and clean up after a failed publish (these land in `~/.local/bin`, so the litter is executable). TDD: both cleanup tests were watched failing with the leftover named in the assertion.

Two more filed rather than fixed: [#512](https://github.com/hherb/kastellan/issues/512) (`kastellan-cli install` prints systemd-only guidance and health-checks via `systemctl` **on macOS** — the new `--no-start` message is `cfg`-gated, its neighbours at `run.rs:138/141/185` and `service_state` are not) and [#513](https://github.com/hherb/kastellan/issues/513) (per-file atomicity is not per-*install* atomicity: two overlapping installs can still publish a mix of both runs' binaries and units — an advisory lock file is the right-sized fix). Declined again: the `write`/`fsync` cleanup seam stays untested, reachable only from ENOSPC/EIO, which no hermetic test can force portably — said so in the test.
...
| --- | --- | --- | --- | --- |
| DGX (native aarch64, real bwrap + KVM + live PG 18) | `f57db609` (`fix/509-review-followups`) | **2965 / 0 / 53** across 162 binaries, `TEST_EXIT=0` | `CLIPPY_EXIT=0` (whole workspace) | exactly 4, all `KASTELLAN_GLINER_RELEX_ENABLE` |
