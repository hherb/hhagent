...
`main` is at **`d37df573`**; the only other open kastellan PR is the unrelated dependabot [#453](https://github.com/hherb/kastellan/pull/453). Everything below the last three merges is settled: the VM-entry arc is COMPLETE, v0.2.0 is RELEASED, the audit-remediation family is FULLY closed, the mail-worker test/e2e arc is done, and the extra-CA capability, its operator config and its live confirmation all shipped.

**Active branch `fix/508-systemd-enable-units` — kastellan now survives a reboot on Linux ([#508](https://github.com/hherb/kastellan/issues/508)).** `supervisor/src/systemd_user.rs` wrote correct `[Install] WantedBy=` stanzas and then never ran `systemctl --user enable`, so nothing was ever linked into `default.target` and a rebooted host came up running nothing — while macOS came back, because `RunAtLoad=true` is unconditional in every plist. That parity break is what left a real Matrix message unanswered on 2026-08-02. `install` now enables `<name>.service`; `install_target` enables `<name>.target` **only** — it is the boot entry point, and its own `Wants=` line pulls the members in, so a second link for each member would be redundant (pinned by a test so a later change is deliberate); `uninstall_target` disables symmetrically, or a real uninstall would strand a `default.target.wants/` symlink. Both calls sit behind the existing `is_default_units_dir()` guard, and `enable` failures propagate rather than being swallowed — a silent failure here reproduces the exact bug, and the operator would not find out until the next reboot. The `Supervisor` trait now carries **"install implies auto-start at boot"** as an explicit cross-platform contract, and `install/run.rs` documents `loginctl enable-linger` as the *other* half: lingering starts the user manager, enabling gives it something to start; neither works alone. **Also fixed here, found by the new tests:** `unique()`/`unique_service_name()` in the two smoke suites derived names from `pid + nanos` only, and every test in a binary shares both a pid and a prefix — so adding a second concurrent test made two of them read the same clock tick, produce the same unit name, and race on one `<name>.service.tmp`, where the loser's rename fails `ENOENT` and surfaces as a bogus `install_target` I/O error in an *unrelated* test. Both now use a process-wide `AtomicU64`, matching `TestRoot` in `src/systemd_user/tests.rs` ([#104](https://github.com/hherb/kastellan/issues/104) tracks the same pattern elsewhere). **Gate: DGX 2952 / 0 / 53** — exactly +2 over the 2950 baseline, the two tests added.
...
2. **Deployment: kastellan did not survive a reboot on Linux.** The missing `systemctl --user enable` — **fixed 2026-08-03 on `fix/508-systemd-enable-units`**, see Current state. The DGX had been hand-`enable`d as a workaround, so its units already read `enabled`; a fresh install elsewhere had the bug until this landed.
...
- **`supervisor`** — `systemd --user` + launchd drivers and unit/plist generation; `core_service_spec` bakes `KASTELLAN_EGRESS_FORCE_ROUTING=1` into the generated unit. **Contract: install implies auto-start at boot** — systemd via `systemctl --user enable` (added in #508), launchd via the unconditional `RunAtLoad=true`; `uninstall` must undo it. Honouring it on one OS only is a parity break.
