...
**Last updated:** 2026-08-27 · **`main` HEAD:** `3bd45a36` ([#623](https://github.com/hherb/kastellan/pull/623), merged 2026-08-25 — the retrospective review of #619) · **OPEN BRANCH:** `claude/headlong-kastellan-integration-094maq` — **docs only, no code change**: the Headlong cross-project study (`docs/devel/notes/2026-08-27-headlong-borrowings.md`) plus the two issues it filed, [#628](https://github.com/hherb/kastellan/issues/628) and [#629](https://github.com/hherb/kastellan/issues/629). Nothing to re-gate; the test baseline below is unchanged. · **DEPLOYED TO THE DGX 2026-08-25** and verified at the binary (`strings` for era markers) and end to end (task 178, a live `web.fetch` guard row carrying `error_kind: null`). The live host had been running the `8736f559`-era build since 2026-08-23 — neither #614 nor #619 — so it really was losing the guard score above 4 KiB until now. · ⚠️ **NEW, found during that deploy: [#624](https://github.com/hherb/kastellan/issues/624) — the D9 boot probe measures daemon-STARTUP CONTENTION, not the host.** Three boots on one unchanged backend derived 21752 / 120000 / 83489 ms from 6073 / 269.6 / 1582 tok/s, while the backend measured ~7000 tok/s uncontended and reproducible. The 269.6 run clamped to the ceiling and fired a **false** "this host cannot adjudicate a worst-case document" coverage finding. `timeout_basis: "probed"` is therefore not reproducible, and a reader treating it as a property of the host is reading noise.
...
- **Deployment — DONE 2026-08-25, verified at the binary.** (History, because the *method* is the
  reusable part: #619's prune had turned a pending action into the past tense — "it kept losing the
  score until `45d5f6c2` was deployed" — and that was false. The DGX checkout read `8cb8cfb7`, which
  looks current, while the running daemon predated it by ~18 h: the tree was pulled and the daemon
  never rebuilt. `strings` on the *installed binary* settled it — `_dropped_preserved` and
  `error_kind` both `0` — which beats every timestamp argument
  [[handover-claims-verify-before-carrying]].)
  `scripts/upgrade_from_git.sh` ran clean: `installed 15 binaries`, operator overlay **11 keys**
  applied after the generated file so nothing was clobbered (the four-key re-add is obsolete —
  `.local` now holds the model tag, the guard three, mail, extra-CA and the Matrix three), Matrix
  channel back up, `KASTELLAN_EGRESS_FORCE_ROUTING=1` confirmed in `/proc/<MainPID>/environ` along
  with the `-ctx64k` model tag. Post-deploy the installed binary carries **all five** era markers
  (`guard_tier.boot`, `_dropped_preserved`, `error_kind`, `connect_timeout`, `operator-below-floor`).
  **End-to-end live check:** task 178 completed, and its `web.fetch` audit row reads
  `{"p": 0.0081, "state": "clear", "error_kind": null, …}` — the #616/#619 schema live, emitted on a
  row where no call failed, which is the shape the whole absence-vs-loss argument turns on.
