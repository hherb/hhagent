...
| --- | --- | --- | --- | --- |
| **DGX** (native aarch64, real bwrap + KVM + live PG 18) | **`76da4bc9`** — `feat/guard-measurement-3-campaign`, the measurement-3 tip | **3754 / 0 / 54**, `TEST_EXIT=0`, `--no-fail-fast --nocapture`, **174 suites**. Reconciles exactly: `main` 3749 **+5**, the five `render_per_case` tests, which are the branch's only new `#[test]`s (everything else it adds is corpus metadata and docs) | `CLIPPY_EXIT=0`, **zero warnings**, over **4 `Checking` lines** — `core` holds both changed files, plus `llm-router`/`tests-common`/`db` in the dev graph, so this is the complete reverse-dep set for the diff rather than the warm-cache false green. **CI stays the authority on lints** (rust 1.97 vs local 1.96) | **8**, all gliner-relex (4 venv-not-built + 4 `KASTELLAN_GLINER_RELEX_ENABLE`) — *not* the bwrap-userns skip, so containment really ran |
