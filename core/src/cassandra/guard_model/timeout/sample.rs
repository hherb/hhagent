...
//! measurement of this backend is**, as distinct from how several of
//! them become one ([`super::summary`]), from the arithmetic that turns
//! that one into a budget ([`super::derive_guard_timeout`]), and from
//! how the budget describes its own provenance ([`super::basis`]).
//!
//! Split again at its own `#624` divider when #625's review pushed it
//! back over the cap — everything about *many* samples now lives in
//! [`super::summary`], leaving this file the singular question its name
//! already claimed.
//!
...
//! other historic path still resolves; both splits moved code, not
//! names.
...
/// together by
/// `the_probe_body_is_the_measured_size_and_carries_no_prose` instead.
/// Changing this number alone changes no behaviour.
///
...
///
/// See [`probe_document`], which prefixes a varying string so the
/// sample is cold — per *sample* since #624, not merely per boot.
...
pub enum ProbeOutcome {
    ...
    ///
    /// **`why` is for the log, not for the row.** It is written by
    /// `tier::probe::run_one_sample`'s `warn!` and then discarded:
    /// [`super::derive_guard_timeout`] maps this to the reason-less
    /// [`super::UnprobedReason::Failed`], because the durable
    /// `timeout_basis` token is deliberately free of backend-supplied
    /// text (the same rule `GuardReport::error_kind` follows). Carrying
    /// the closed [`super::super::tier::GuardErrorKind`] here instead,
    /// so the row could say *which* failure, is worth doing and is not
    /// done.
    Failed { why: String },
}

/// The probe document: a `cache_buster` followed by [`PROBE_BODY`].
///
/// The caller supplies the buster. Since issue #624 that is
/// [`super::summary::sample_cache_buster`]'s per-**sample** value, not a
/// per-boot one —
/// the document must differ from the last boot's *and* from this boot's
/// other samples, for the reason recorded there.
///
...
/// exists only to make this sample's prompt differ from every other one
/// this backend has lately seen: the last boot's, and this boot's other
/// samples. Naming it a nonce overstates its role to a reader, and CodeQL's
