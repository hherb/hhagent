...
//!
//! Pure throughout — no server, no clock, no Postgres. Most of these
//! were lifted out of `timeout/tests.rs` when `timeout.rs` was split, so
//! that each production file carries the tests that name it. Living here
//! is also what gives them the private [`PROBE_BODY`] without widening
//! its visibility for a test's convenience.
//!
//! **Not "unchanged", as this said until #625's review diffed it**: one
//! lifted body had to inline a fixture the child module cannot reach
//! (see `a_cache_contaminated_sample_is_rejected_rather_than_believed`,
//! which says so eleven lines down and contradicted this header), and
//! everything below the `#624` rule is new. The claim that survived the
//! split intact is the *name set* — 27 `#[test]` before and the same 27
//! after — which is what the split commit asserts.
...
// `summarise` lives in the sibling `summary` module since #625's review
// split this file; reached through the parent's re-export, so
// `derive_from_one` keeps exercising the real path rather than a
// hand-built summary.
use super::super::{derive_guard_timeout, summarise, GuardTimeout, TIMEOUT_FLOOR_MS};
use super::*;
